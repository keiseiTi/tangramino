...
```bash
npm install loro-crdt socket.io-client
```
...
- **Loro**：不同 CRDT 库的 API 差异巨大（Loro、Yjs、Automerge），应用层无法通过简单配置替换
- **Socket.IO**：通信层接口相对标准，应用层可以用 WebSocket、WebRTC、HTTP 轮询等方案替换
  :::
