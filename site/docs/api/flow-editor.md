...
function AttributePanel() {
  ...
  if (!activeNode) {
    return <div className="empty">选择节点以编辑属性</div>;
  }
...
function ToolBar() {
  ...
  return (
    <div className="toolbar">
      <button onClick={() => ctx.tools.zoomIn()}>+</button>
      <button onClick={() => ctx.tools.zoomOut()}>-</button>
      <button onClick={() => ctx.tools.fitView()}>适应</button>
      <button onClick={() => ctx.history.undo()}>撤销</button>
      <button onClick={() => ctx.history.redo()}>重做</button>
    </div>
  );
}
...
function FlowEditorApp() {
  ...
  return (
    <FlowEditor
      ...
      canDeleteNode={(ctx, node) => node.type !== 'start'}
    >
